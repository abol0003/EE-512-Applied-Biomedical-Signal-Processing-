...
%design the IIR bandpass filter [4-8]Hz
...
figure
subplot(511), plot(t, IF_Hilbert), title('IF\_Hilbert on raw V3 signal'), xlabel('Time [s]')
subplot(512), plot(t, IF_Teager), title('IF\_Teager on raw V3 signal'), xlabel('Time [s]')
...
IF_Hilbert_f = IFhilbert(V3_filt,50);
[IF_Teager_f,env] = teager(V3_filt,50);
IF_STFT_f = STFT(V3_filt,31,50);
[IF_Adapt_1f,y] = AdaptBP(V3_filt,5,0.85,0.9,50,0);
[IF_Adapt_2f,y] = AdaptBP(V3_filt,5,0.98,0.9,50,0);

figure
subplot(511), plot(t, IF_Hilbert_f), title('IF\_Hilbert on filtered V3 signal'), xlabel('Time [s]')
subplot(512), plot(t, IF_Teager_f), title('IF\_Teager on filtered V3 signal'), xlabel('Time [s]')
subplot(513), plot(t, IF_STFT_f), title('IF\_STFT on filtered V3 signal'), xlabel('Time [s]'), ylim([0 15])
...
%%
% --------- Experiment 2 -------------
% estimation of respiration frequency from respiratory sinus arrhythmia during a VO2-max test
